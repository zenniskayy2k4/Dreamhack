...
def get_uint32(b: bytes) -> int:
    return (b[0] << 24) | (b[1] << 16) | (b[2] << 8) | (b[3])


def put_uint32(val: int) -> bytes:
    return bytes([(val >> 24) & 0xFF, (val >> 16) & 0xFF, (val >> 8) & 0xFF, val & 0xFF])


def fi(fin: int, fkey: int) -> int:
    d9 = (fin >> 7) & 0x1ff
    d7 = fin & 0x7f
    d9 = s9table[d9] ^ d7
    d7 = s7table[d7] ^ d9
...
class Misty1:
    def __init__(self, key: bytes):
        ...
        self.ek = [0] * 32
        for i in range(8):
            self.ek[i] = (key[i*2] << 8) + key[i*2+1]
        for i in range(8):
            self.ek[i+8] = fi(self.ek[i], self.ek[(i+1) % 8])
            self.ek[i+16] = self.ek[i+8] & 0x1ff
            self.ek[i+24] = self.ek[i+8] >> 9

    def block_size(self):
        return 8

    def fo(self, fin: int, k: int) -> int:
        t0 = (fin >> 16) & 0xFFFF
        t1 = fin & 0xFFFF
        t0 = t0 ^ self.ek[k]
        t0 = fi(t0, self.ek[(k+5) % 8+8])
        t0 = t0 ^ t1
        t1 = t1 ^ self.ek[(k+2) % 8]
        t1 = fi(t1, self.ek[(k+1) % 8+8])
        t1 = t1 ^ t0
        t0 = t0 ^ self.ek[(k+7) % 8]
        t0 = fi(t0, self.ek[(k+3) % 8+8])
        t0 = t0 ^ t1
        t1 = t1 ^ self.ek[(k+4) % 8]
        fout = ((t1 << 16) | t0) & 0xFFFFFFFF
        return fout

    def fl(self, fin: int, k: int) -> int:
        d0 = (fin >> 16) & 0xFFFF
        d1 = fin & 0xFFFF
        if k & 1 == 0:
            d1 = d1 ^ (d0 & self.ek[k//2])
            d0 = d0 ^ (d1 | self.ek[(k//2+6) % 8+8])
        else:
            d1 = d1 ^ (d0 & self.ek[((k-1)//2+2) % 8+8])
            d0 = d0 ^ (d1 | self.ek[((k-1)//2+4) % 8])
        fout = ((d0 << 16) | d1) & 0xFFFFFFFF
        return fout

    def flinv(self, fin: int, k: int) -> int:
        d0 = (fin >> 16) & 0xFFFF
        d1 = fin & 0xFFFF
        if k & 1 == 0:
            d0 = d0 ^ (d1 | self.ek[(k//2+6) % 8+8])
    ...
    def encrypt_block(self, src: bytes) -> bytes:
        d0 = get_uint32(src[:4])
        d1 = get_uint32(src[4:])
        ...
        return put_uint32(d1) + put_uint32(d0)
